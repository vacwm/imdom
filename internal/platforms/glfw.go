...
func NewGLFW(io imgui.IO) (*GLFW, error) {
	...
	window, err := glfw.CreateWindow(windowWidth, windowHeight, "Immediate Mode DOM", nil, nil)
	if err != nil {
		glfw.Terminate()
...
// ShouldStop returns true is the window is to be closed.
...
// DisplaySize returns the dimensions of the display.
func (platform *GLFW) DisplaySize() [2]float32 {
	w, h := platform.window.GetSize()
	return [2]float32{float32(w), float32(h)}
}

// FramebufferSize returns the dimensions of the framebuffer.
